						...
						if(!flag){	//Verify input password vs user actual password
							...
							Eclair.eclair.receive(msat, desc,(function(err, res1){
								...
								qrcode.toDataURL(ln_req, async (err1, qr) => {
									...
									myurl = qr;
									console.log(ln_req);
									const recipient = {"to": req.session.userData.ID, "ln_req": ln_req};
									string.pending.push(recipient);
									const postdata = JSON.stringify(string, null, 2);
									fs.writeFileSync("./src/pool.json", postdata);
