			...
			if(string){	//Make sure user infor is not empty
				...
				if(!flag){	//Verify input password vs user actual password
					...
					if(sat>string.BTC.ln_Balance){	//Check user balance
						...
						const filename2 = "./src/"+receive_ID;
						const file2 = filename2+".json";
						fs.open(file2, "r", (err, fd) => {
							...
							const rr2 = fs.createReadStream(file2);
							var str = "";
							rr2.on("data", (chunk2) => {
								str = JSON.parse(chunk2);
							});
							rr2.on("end", () => {
								if(str) {
									const now = new Date();
									const jsondate = now.toJSON();
									function sum(a,b){return a/1 + b;}
									str.BTC.ln_Balance = sum(str.BTC.ln_Balance , sat);	//Update Recipient balance
									...
									const recipient = {"type": "Internal", "currency": "ln_btc", "from": req.session.userData.ID, "amount": sat, "desc": desc, "timereceived": jsondate};
									string.sent.push(sender);
									str.received.push(recipient);
									const postdata1 = JSON.stringify(string, null, 2);
									const postdata2 = JSON.stringify(str, null, 2);
									fs.writeFileSync(file2, postdata2);
									encryption.encrypt({filename: filename2, password: "Backintime123"});
									...
									console.log("Can't read File 2 or File 2 is empty");
									rr1.destroy();
								}
							});
							rr2.on("close", () => { //Close stream 2
								/*fs.unlinkSync(file2);
								console.log('File 2 deleted!'); */
								rr1.destroy();
								console.log("Stream 2 is closed");
							});
						});
					}
				}
